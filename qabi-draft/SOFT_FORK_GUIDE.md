...
How Ladder Script activates as a soft fork on Bitcoin. All 63 block types activate
...
Live signet at `85.9.213.194` with all 63 active block types verified end-to-end.
...
**Status:** All 63 block types have fund+spend proof with recorded transaction IDs.
...
- All 62 Ladder Script block types activate simultaneously. No phased block type rollout.
...
Flag byte 0x02 signals RUNG_TX serialization format
```
...
| Signet verification | 63/63 | All active block types: fund + mine + spend on live signet with recorded txids |
...
| Engine smoke tests | 48 | 48 templates, 63 block types, getTypeHex coverage, dead code checks |
