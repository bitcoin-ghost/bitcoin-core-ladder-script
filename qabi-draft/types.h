 ...
 *    0x0001-0x00FF  Signature family (SIG, MULTISIG, ADAPTOR_SIG, MUSIG_THRESHOLD, KEY_REF_SIG, QABI)
...
enum class RungBlockType : uint16_t {
    ...
    QABI             = 0x0006, //!< Quantum Atomic Batch Input — PQ-only signature anchor
...
inline std::string BlockTypeName(RungBlockType type)
{
    ...
    case RungBlockType::QABI:             return "QABI";
...
/** QABI conditions: [SCHEME(1)] — PQ-only signature anchor. Same layout as SIG;
 *  PQ-only enforcement is at the evaluator level (rejects classical schemes). */
inline constexpr ImplicitFieldLayout QABI_CONDITIONS = SIG_CONDITIONS;
...
/** QABI witness: [PUBKEY, SIGNATURE] — PQ-only, same wire format as SIG */
inline constexpr ImplicitFieldLayout QABI_WITNESS = SIG_WITNESS;
...
inline const ImplicitFieldLayout& GetImplicitLayout(RungBlockType type, uint8_t ctx)
{
    ...
    if (ctx == 1) {
        ...
        case RungBlockType::QABI:             return QABI_CONDITIONS;
        ...
        case RungBlockType::QABI:             return QABI_WITNESS;
...
inline const BlockDescriptor* LookupBlockDescriptor(RungBlockType type)
{
    ...
    static const BlockDescriptor BLOCK_DESCRIPTORS[] = {
        ...
        {RungBlockType::QABI, "QABI", true, false, true, 1, &QABI_CONDITIONS, &QABI_WITNESS, false},
        // Timelock family
