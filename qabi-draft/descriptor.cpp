...
bool ParseQabi(ParseContext& ctx, RungBlock& block, std::vector<std::vector<uint8_t>>& rung_pks)
{
    // qabi(@alias) or qabi(@alias, scheme)
    // QABI is PQ-only: defaults to FALCON512, rejects classical schemes.
    if (!Expect(ctx, '(')) return false;
    std::string alias = ReadAlias(ctx);
    if (alias.empty()) return false;
    ...
    block.type = RungBlockType::QABI;
    RungScheme scheme = RungScheme::FALCON512; // default PQ scheme
    ...
    // QABI enforces PQ-only
    if (!IsPQScheme(scheme)) {
        ctx.error = "QABI requires a post-quantum scheme (falcon512, falcon1024, dilithium3, sphincs_sha)";
        return false;
    }
...
bool ParseBlock(ParseContext& ctx, RungBlock& block, std::vector<std::vector<uint8_t>>& rung_pks)
{
    ...
    else if (name == "qabi") ok = ParseQabi(ctx, block, rung_pks);
    // Timelock family
