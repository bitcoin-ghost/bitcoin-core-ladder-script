...
static bool ParseBlockType(const std::string& name, RungBlockType& out)
{
    ...
    if (name == "QABI")             { out = RungBlockType::QABI; return true; }
    // Timelock family
                                   ...
                                   const RungConditions& conditions)
{
    ...
    case RungBlockType::QABI: {
        // QABI: PQ-only signature anchor. Must use a PQ scheme — reject classical.
        if (block_spec.exists("privkey") && !block_spec.exists("scheme")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                "QABI requires a PQ scheme (e.g. \"scheme\": \"FALCON512\"). "
                "Classical signing (bare privkey) is not allowed.");
        }
        if (block_spec.exists("scheme")) {
            std::string scheme_str = block_spec["scheme"].get_str();
            RungScheme scheme;
            if (!ParsePQScheme(scheme_str, scheme)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                    "QABI: scheme '" + scheme_str + "' is not a post-quantum scheme. "
                    "Use FALCON512, FALCON1024, DILITHIUM3, or SPHINCS_SHA.");
            }
        }
        SignSingleKey(block_spec, block, mtx, input_idx, txdata, conditions, "QABI");
        break;
    }
