...
raw opcodes with 63 typed function blocks organised into 10 families. Every
...
| Covenant support | 63 native block types | None | Full covenant system |
...
Ladder Script defines 63 block types across 10 families. Each block type is
...
| `0x0006` | QABI | Quantum Atomic Batch Input — PQ-only signature anchor |
...
All 63 block types activate together as a single soft fork. Partial
...
| `evaluator.cpp` | All 63 block evaluators, `EvalBlock`, `EvalLadder`, `VerifyRungTx` |
...
62/63 block types verified on signet.
...
Detailed evaluation semantics for all 63 block types. Dispatch in
...
| QABI | Quantum Atomic Batch Input. PQ-only signature anchor — rejects classical schemes (Schnorr/ECDSA). Verify PQ SIGNATURE against PUBKEY using SCHEME. Enables O(1) quantum signature overhead for N-party transactions. |
...
All 63 block types will be exercised on signet with fund+spend transaction
