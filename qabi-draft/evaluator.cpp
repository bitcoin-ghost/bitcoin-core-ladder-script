...
// ============================================================================
// QABI evaluator (Quantum Atomic Batch Input — PQ-only)
// ============================================================================

EvalResult EvalQabiBlock(const RungBlock& block,
                          const BaseSignatureChecker& checker,
                          SigVersion sigversion,
                          ScriptExecutionData& execdata)
{
    // QABI: PQ-only signature anchor. Same as SIG but rejects classical schemes.
    // Conditions: SCHEME (must be PQ). Witness: PUBKEY, SIGNATURE.
    const RungField* pubkey_field = FindField(block, RungDataType::PUBKEY);
    const RungField* sig_field = FindField(block, RungDataType::SIGNATURE);
    const RungField* scheme_field = FindField(block, RungDataType::SCHEME);

    if (!pubkey_field || !sig_field) return EvalResult::ERROR;
    if (!scheme_field || scheme_field->data.empty()) return EvalResult::ERROR;

    auto scheme = static_cast<RungScheme>(scheme_field->data[0]);

    // QABI enforces PQ-only: reject classical schemes (Schnorr, ECDSA)
    if (!IsPQScheme(scheme)) {
        return EvalResult::UNSATISFIED;
    }

    // Verify PQ signature (reuses existing PQ verifier)
    return EvalPQSig(scheme, *sig_field, *pubkey_field, checker);
}
                     ...
                     int depth)
{
    ...
    case RungBlockType::QABI:
        raw = EvalQabiBlock(block, checker, sigversion, execdata);
        break;
    // Timelock
