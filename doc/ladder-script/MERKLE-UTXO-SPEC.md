...
```cpp
static constexpr uint8_t RUNG_MLSC_PREFIX = 0xdf;      // conditions.h (MLSC prefix)
...
All transaction outputs must use valid MLSC format (`0xDF`). Non-Ladder
...
|----------|-------|----------|-------------|
| `RUNG_MLSC_PREFIX` | `0xDF` | `conditions.h` | MLSC prefix byte |
